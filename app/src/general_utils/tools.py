"""Shared tools available to every sub-agent: a reasoning ``think`` scratchpad and a deterministic
...
its call budget from the injected runtime context, so it stays decoupled from any specific agent via
the :class:`ComputeRuntimeContext` protocol.
"""
...
class ComputeRuntimeContext(Protocol):
    """Structural contract the ``compute`` tool needs from any agent's runtime context."""
